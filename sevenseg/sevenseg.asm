;  ASM code generated by mikroVirtualMachine for PIC - V. 8.0.0.0
;  Date/Time: 5/12/2015 3:15:07 AM
;  Info: http://www.mikroe.com


; ADDRESS	OPCODE	ASM
; ----------------------------------------------
$0000	$2804			GOTO	_main
$0004	$	_main:
;sevenseg.c,1 :: 		void main()
;sevenseg.c,4 :: 		TRISB=0b00001111;
$0004	$300F			MOVLW	15
$0005	$1303			BCF	STATUS, RP1
$0006	$1683			BSF	STATUS, RP0
$0007	$0086			MOVWF	TRISB
;sevenseg.c,5 :: 		PORTB.F0=0;
$0008	$1283			BCF	STATUS, RP0
$0009	$1006			BCF	PORTB, 0
;sevenseg.c,6 :: 		PORTB.F1=0;
$000A	$1086			BCF	PORTB, 1
;sevenseg.c,7 :: 		PORTB.F2=0;
$000B	$1106			BCF	PORTB, 2
;sevenseg.c,8 :: 		PORTB.F3=0;
$000C	$1186			BCF	PORTB, 3
;sevenseg.c,10 :: 		while(1)
$000D	$	L_main_0:
;sevenseg.c,12 :: 		PORTB.F7=PORTB.F0;
$000D	$3001			MOVLW	1
$000E	$0506			ANDWF	PORTB, 0
$000F	$00F0			MOVWF	STACK_0
$0010	$3000			MOVLW	0
$0011	$1870			BTFSC	STACK_0, 0
$0012	$3080			MOVLW	128
$0013	$0606			XORWF	PORTB, 0
$0014	$3980			ANDLW	128
$0015	$0686			XORWF	PORTB, 1
;sevenseg.c,13 :: 		PORTB.F6=PORTB.F1;
$0016	$3000			MOVLW	0
$0017	$1886			BTFSC	PORTB, 1
$0018	$3001			MOVLW	1
$0019	$00F1			MOVWF	STACK_1
$001A	$3000			MOVLW	0
$001B	$1871			BTFSC	STACK_1, 0
$001C	$3040			MOVLW	64
$001D	$0606			XORWF	PORTB, 0
$001E	$3940			ANDLW	64
$001F	$0686			XORWF	PORTB, 1
;sevenseg.c,14 :: 		PORTB.F5=PORTB.F2;
$0020	$3000			MOVLW	0
$0021	$1906			BTFSC	PORTB, 2
$0022	$3001			MOVLW	1
$0023	$00F1			MOVWF	STACK_1
$0024	$3000			MOVLW	0
$0025	$1871			BTFSC	STACK_1, 0
$0026	$3020			MOVLW	32
$0027	$0606			XORWF	PORTB, 0
$0028	$3920			ANDLW	32
$0029	$0686			XORWF	PORTB, 1
;sevenseg.c,15 :: 		PORTB.F4=PORTB.F3;
$002A	$3000			MOVLW	0
$002B	$1986			BTFSC	PORTB, 3
$002C	$3001			MOVLW	1
$002D	$00F1			MOVWF	STACK_1
$002E	$3000			MOVLW	0
$002F	$1871			BTFSC	STACK_1, 0
$0030	$3010			MOVLW	16
$0031	$0606			XORWF	PORTB, 0
$0032	$3910			ANDLW	16
$0033	$0686			XORWF	PORTB, 1
;sevenseg.c,16 :: 		}
$0034	$280D			GOTO	L_main_0
;sevenseg.c,18 :: 		}
$0035	$2835			GOTO	$
